      ...
      <ScrollArea className="w-full">
        <div className="flex w-max space-x-4 p-4 pt-0">
          {restaurant.menuCategories.map((category) => (
            <Button onClick={() => handleCategoryClick(category)} key={category.id} variant={selectCategory.id === category.id ? 'default' : 'secondary'} size="sm" className="rounded-full">{category.name}</Button>
          ))}
        </div>
